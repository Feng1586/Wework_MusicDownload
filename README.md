...
冒号后面是**当前队列的任务数**（含正在下载的那首），不是选中的编号：
...
每首歌下载完成后，旁边会自动多出**同名的歌词和封面**，并且这两样也会
**嵌入音频文件自己的标签里**（换机器、只拷音频文件也不会丢）：

```
downloads/QQMusicClient/2026-09-21-00-28-01 青花瓷/
├── 青花瓷.flac      ← 音频；标签里已含歌词与封面
├── 青花瓷.lrc       ← 歌词（UTF-8，标准 LRC 带时间轴）
└── 青花瓷.jpg       ← 封面（800×800 JPEG）
```

几点说明：

- 歌词和封面**是后端随搜索结果一起返回的**，不会为此多发一次请求。
- 用同名 `.lrc` / `.jpg` 而不是 `cover.jpg` / `folder.jpg`：同一批次目录里
  可能落好几首歌，固定名字会互相覆盖。
- 支持嵌入 `flac`（`LYRICS` / `UNSYNCEDLYRICS` + `PICTURE`）、
  `mp3`（`USLT` + `APIC`，存为 ID3v2.3）、`m4a`（`©lyr` + `covr`）；
  其他格式只写伴随文件。
- 封面拿不到、或嵌入失败，**只记日志，不会让下载算失败** —— 伴随文件是独立写好的。
- 这首歌词里原本空的 `[ar:]` 会用歌手名补上（`[ti:]` 已有值时不动），
  否则播放器显示不出歌手。
...
只会收到一条汇总回执，数字是本批加入后的队列总数，然后按顺序一首一首下载：
